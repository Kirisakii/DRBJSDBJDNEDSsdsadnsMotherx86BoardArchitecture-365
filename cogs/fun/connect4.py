...
class Connect4Game:
    ...
    def move(self, column):
        adjusted_column = column - 1  # Subtract 1 to convert to 0-based index
        if adjusted_column < 0 or adjusted_column >= self.board.width:
            raise ValueError("Invalid column number.")
        self.board[adjusted_column] = self.whomst_turn()
        self.turn_count += 1
    ...
    async def connect4(self, interaction: discord.Interaction, player2: discord.Member):
        ...
        while game.whomst_won() == game.NO_WINNER:
            ...
            if str(reaction) == self.CANCEL_GAME_EMOJI:
                ...
                break
            try:
                column_index = self.DIGITS.index(str(reaction))
                game.move(column_index + 1)
            except ValueError:
