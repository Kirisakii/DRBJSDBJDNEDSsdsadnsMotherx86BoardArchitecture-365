...
from cogs.utility.roles import RoleMenuView  # Import the RoleMenuView class
# import logging
...
class DeleteEmbedView(discord.ui.View):
    ...
    async def remove_embed(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        success, message = await embed_project_cog.delete_buttons_for_embed(self.user_id, self.embed_name)
        if not success:
...
class RemoveButtonModal(discord.ui.Modal, title="Remove a Button"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        success, message = await embed_project_cog.delete_button_info_json(self.user_id, custom_id)
        if success:
...
class FinalizeButtonView(discord.ui.View):
    ...
    async def confirm_button_creation(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if embed_project:
            ...
            custom_id = await embed_project.add_button_info_json(
...
class DynamicButtonView(discord.ui.View):
    ...
    def load_all_buttons(self):
        try:
            with open('db/buttons.json', 'r') as f:
                data = json.load(f)
                # Iterate over all embed names and load buttons
                for embed_name, config in data.items():
                    if embed_name == "last_id":
                        continue  # Skip the 'last_id' field
                    self.load_buttons_for_embed(embed_name, config["buttons"])
        except Exception as e:
            print(f"Failed to load buttons: {str(e)}")

    def load_buttons_for_embed(self, embed_name, button_configs):
        for button_config in button_configs:
            if button_config['style'].lower() == 'link' and button_config.get('link'):
                # Create a link button directly
                button = discord.ui.Button(label=button_config['label'], style=discord.ButtonStyle.link, url=button_config['link'])
            else:
                # Create a regular button that can trigger interactions
                button = DynamicButton(button_config, self.bot)
...
class DynamicButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = None
        if embed_name:
            embed = await self.bot.get_cog("EmbedProject").retrieve_embed_data(creator_user_id, embed_name)
        ...
        if self.config.get('custom_message'):
            if embed:
                await interaction.response.send_message(self.config['custom_message'], embed=embed, ephemeral=True)
            else:
...
class EmbedProject(commands.Cog):
    ...
    async def retrieve_embed_data(self, user_id: int, embed_name: str, include_view=False):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT * FROM user_embeds WHERE user_id = ? AND embed_name = ?", (user_id, embed_name)) as cursor:
                ...
                if include_view:
                    # Check if there are button configurations for this embed that belong to the user
                    try:
                        with open('db/buttons.json', 'r') as f:
                            buttons_data = json.load(f)
                            user_buttons = buttons_data.get(embed_name, {}).get("buttons", [])
                            view = discord.ui.View()
                            for button in user_buttons:
                                if str(button["user_id"]) == str(user_id):  # Ensure button belongs to the user
                                    view.add_item(DynamicButton(button, self.bot))  # Pass bot to DynamicButton
                    except FileNotFoundError:
                        view = discord.ui.View()  # Return an empty view if the file is not found

                    return embed, view
                else:
                    return embed
    async def delete_button_info_json(self, user_id: int, custom_id: str):
        try:
            with open('db/buttons.json', 'r') as file:
                buttons_data = json.load(file)
        except FileNotFoundError:
            return False, "Button configuration file not found."

        # Find and remove the button with the matching custom_id and user_id
        button_found = False
        for embed_name, config in buttons_data.items():
            if embed_name == "last_id":
                continue
            buttons = config.get("buttons", [])
            for button in buttons:
                if button["custom_id"] == custom_id and str(button["user_id"]) == str(user_id):
        ...
        with open('db/buttons.json', 'w') as file:
            json.dump(buttons_data, file, indent=4)
    ...
    async def add_button_info_json(self, user_id, embed_name, button_label, button_style, link=None, emoji=None, role_to_give=None, send_embed=None, custom_message=None, guild_id=None, role_menu_name=None):
    # Load existing button configurations
        try:
            with open('db/buttons.json', 'r') as file:
                buttons_data = json.load(file)
                last_id = buttons_data.get("last_id", 0)
        except FileNotFoundError:
            buttons_data = {}
            last_id = 0

        # Increment the last_id for a new custom_id
        new_id = last_id + 1
        custom_id = f"{new_id:06d}"  # Formats the ID as a zero-padded string

        # Prepare the new button configuration
        new_button = {
            "user_id": user_id,
            ...
            "guild_id": guild_id,  # Include guild_id
            "role_menu_name": role_menu_name,  # Include role_menu_name
            "custom_id": custom_id
        }

        # Add the new button to the embed's button list
        if embed_name not in buttons_data:
            buttons_data[embed_name] = {"buttons": []}
        buttons_data[embed_name]["buttons"].append(new_button)
        buttons_data["last_id"] = new_id  # Update the last_id in the JSON
    ...
    async def delete_buttons_for_embed(self, user_id: int, embed_name: str):
        try:
            with open('db/buttons.json', 'r') as file:
                buttons_data = json.load(file)
        except FileNotFoundError:
        ...
        # Initialize a flag to check if any buttons are found and deleted
        buttons_found = False

        # Iterate over each embed's buttons and remove those matching the user_id and embed_name
        for key in list(buttons_data.keys()):  # Use list to avoid dictionary changed size during iteration error
            if key == "last_id":
                continue
            embed_buttons = buttons_data[key]["buttons"]
            # Filter out buttons that should be deleted
            original_count = len(embed_buttons)
            embed_buttons[:] = [button for button in embed_buttons if not (str(button["user_id"]) == str(user_id) and button["embed_name"] == embed_name)]
            if len(embed_buttons) != original_count:
                buttons_found = True
                print(f"Deleted buttons from embed: {embed_name}")

        if not buttons_found:
            print("No buttons found for this embed.")
        ...
        print("Buttons deleted successfully.")
    ...
    @app_commands.command(name="send_through_webhook")
    ...
    async def fetch_user_buttons(self, user_id):
        """Fetch buttons for a specific user."""
        try:
            with open('db/buttons.json', 'r') as file:
                buttons_data = json.load(file)
        except FileNotFoundError:
            return None, "No buttons configuration file found."
        ...
        for embed_name, config in buttons_data.items():
            ...
            for button in config.get("buttons", []):
                if str(button["user_id"]) == str(user_id):  # Ensure string comparison
                    user_buttons.append(f"1. {button['label']}\n> **__ID: {button['custom_id']}__**, embed = {button['embed_name']}")

        return user_buttons, None

    @app_commands.command(name="my_buttons")
    ...
    @app_commands.command(name="delete_button")
    ...
    async def delete_button(self, interaction: discord.Interaction, custom_id: str):
        ...
        success, message = await self.delete_button_info_json(user_id, custom_id)
        if not success:
    ...
    async def add_button(
        ...
        custom_id = await self.add_button_info_json(
    ...
    async def delete_embed_command(self, ctx: commands.Context, embed_name: str):
        ...
        success, message = await self.delete_buttons_for_embed(user_id, embed_name)
        if not success:
...
async def setup(bot: commands.Bot):
    ...
    bot.add_view(DynamicButtonView(bot))
