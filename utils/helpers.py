from utils.envkeys import kawaii_red, tenor_key, spotify_id, spotify_secret
...
async def gel():
    ...
    gelb = booru.Gelbooru()
    res = await gelb.search(query=f"{lewd} rating:{lewd_rate}", block="guro", limit=100, page=2, random=True)
