...
import discord
from flags.flags_env import mongo
...
def random_country(mode: int) -> tuple:
    if mode == 1:
...
async def start_game(self, interaction: discord.Interaction, mode: int) -> None:
    ...
    chances = 3  # Variable to track the number of chances left
    ...
    while chances > 0:
        country_name, country_flag = random_country(mode)
        ...
        def check(m):
            # Extract the command part of the message (first word)
            command_part = m.content.split(' ')[0].upper()  # Convert to upper case to standardize
            # Check if the command part is close enough to "ANSWER"
            if fuzz.partial_ratio(command_part, "ANSWER") >= 70:
        ...
        try:
            ...
            if chances == 0:
                ...
                new_stats['$inc']['time_lost'] = 1
                database.update_one(user_filter, new_stats)
            continue
        else:
            # Using fuzzy matching to compare the guessed answer with the correct answer
            guessed_country = guess.content.split(' ', 1)[1].strip().lower()  # Skip the first word which is "ANSWER"
            similarity_score = fuzz.ratio(guessed_country, country_name.lower())
            ...
            if similarity_score >= 80:
                ...
                if chances == 0:
                    ...
                    await interaction.channel.send(f"Wrong answer. You have {chances} chances left.", delete_after=5)
    ...
    suffix = ''
    if str(games_played)[-1] == '1' and games_played != 11:
        suffix = 'st'
    elif str(games_played)[-1] == '2' and games_played != 12:
        suffix = 'nd'
    elif str(games_played)[-1] == '3' and games_played != 13:
        suffix = 'rd'
    else:
        suffix = 'th'
    ...
    statistics = discord.Embed(
        ...
        description=f"Congratulations <@{interaction.user.id}>! You just completed your **{games_played}{suffix}** game with a streak of **{current_streak}** correct guesses! To display additional information about users just run the `/flags statistics` command or `/flags leaderboard` for global statistics.",
        color=0x575fcf,
        timestamp=datetime.datetime.utcnow()
    )
    statistics.set_footer(
        text=f'Requested by {interaction.user.name}',
        icon_url=interaction.user.avatar.url  # Updated to use the correct attribute
    )
